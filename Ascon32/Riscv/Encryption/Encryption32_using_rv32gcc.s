...
######################################################################
######################################################################
#define EXT_BYTE32(x,n) ((u8)((u64)(x)>>(8*(7-(n)))))
   #EXT_BYTE3232(x,n)=EXT_BYTE3232(a0,a1)
   #this function shift right the register x by 8*(3-n) and cut the lowest byte
EXT_BYTE32: addi sp,sp,-8
            sd   t0,0(sp)
      ...
      #return value in a0
######################################################################################     
#define INS_BYTE32(x,n) ((u64)(x)<<(8*(7-(n))))       
#INS_BYTE3232(x,n)=INS_BYTE3232(a0,a1)
...
for_loop:
	bge t4,s10,exit_for  #for (i = 0; i < rlen; ++i, ++ad)
		...
		add a1,t4,0
		jal INS_BYTE32
		xor s0,s0,a0 #x0 ^= INS_BYTE32(*ad, i);
	addi t4,t4,1
	addi a7,a7,1
